...
bodies of inline methods, except for members that are referred to inside meta scopes.
...
declared in the `scala.meta` metaprogramming library:
    ...
    def meta[Repr, Result]
      (body: implicit scala.meta.semantic.Context => Repr)
      (implicit ev: scala.meta.Metafy[Result, Repr]): Result = ???
